...
def calculate_funeral():
    try:
        ...
        member_sheet.range("A2:G1000").clear_contents()  # include G so old covers are wiped
        ...
        for idx, member in enumerate(members):
            ...
            # 🔹 Write Sum Assured as numeric value
            try:
                member_sheet.range(f"G{row}").value = float(member.get("coverAmount") or 0)
            except:
                member_sheet.range(f"G{row}").value = 0

        print("🧝 Member data written to MemberData sheet")

# 🔹 Normalize principalMemberCover to always be a float with 2 decimals
        if inputs.get("principalMemberCover"):
            try:
                inputs["principalMemberCover"] = f"{float(inputs['principalMemberCover']):.2f}"
            except:
                pass
        ...
        print("🔍 Verifying written cover levels:")
        for i in range(17, 27):
            print(f"I{i}: {input_sheet.range(f'I{i}').value}")


       # --- Run Macro ---
        print("⚙️ Running Excel macro 'Pricing'...")

        # 🔹 Force Excel to recalc before macro runs (ensures inputs are recognized)
        wb.app.calculate()
        time.sleep(3)
        ...
        # 🔹 Force full workbook recalc again after macro
        wb.app.calculate()
        time.sleep(3)
